<#
.DESCRIPTION
   This script is to check mailbox permission for a given list of email addresses
   based on the input from GUI
.AUTHOR
    Mohd Zain Hassan (mzainh@gmail.com)
#>
...
$userEmail = GUI_TextBox "Enter email address : "
$emaiCount = $userEmail | Measure-Object | % {$_.Count} ### Measures how many objects were inputted

if ($emaiCount -eq 0) {
    Write-Host "Nothing was inputted.." -BackgroundColor Red -ForegroundColor White
    Return
}
...
Else {
    ...
    foreach ($email in $userEmail) {
        Write-Host "Getting details for " $email
        Get-ExoMailboxPermission -Identity $email
        Write-Host "----------------------------------"
    }
}
